...
- **Handlungspfad (Korrektur 28.08. nach Nutzertest):** jede Organisation trägt ihren **offiziellen Spendenweg** als `datum` (`donation_channel`: URL nur auf der eigenen Domain der Organisation oder auf einer gov.np-Domain; `channel_type`, `flood_specific`, `verification` immer `self_reported`; keine Kontonummern werden gespeichert oder angezeigt, nur der Link auf die Originalseite). Auf dem Board als gleichgewichtiger Link je Zeile („Offizieller Spendenweg"), auf der Org-Seite als eigener Abschnitt mit Provenienz-Chip und dem festen Sicherheitssatz „Kontodaten immer auf der Originalseite der Organisation prüfen. Diese Seite empfiehlt keine Organisation." Orgs ohne Spendenweg zeigen „kein offizieller Spendenweg gefunden" in voller Stärke. Der nepalesische Prime Minister Disaster Relief Fund ist **kein** Responder-Eintrag (er würde die Zählungen 44/44 und „ohne Reaktion" verfälschen), sondern steht als staatlicher Fonds im „Ich möchte helfen"-Abschnitt, gespeist aus `donation-channels.json → government_funds[]` (Korrektur 28.08. spät). Einstieg „Ich möchte helfen" im Masthead. **Kein** „empfohlen", keine imperative Spenden-Copy, keine bevorzugte Button-Gestaltung.
- **Nicht in v1** (benannt, damit es nicht versehentlich gebaut wird): FTS-„Zusage→Auszahlung"-Layer, UK-CC-Peer-Verteilung/Programmquote, Statement-Review-UI, Auth, Nutzerkonten, eigene Spendenabwicklung/Zahlungsintegration.
...
## Datenmodell (Postgres, 14 Tabellen — Kurzform; Details im Spec)
...
- `disaster` (glide_id PK `ff-2026-000162-npl`, reliefweb_id `D52684`), `district` (77 Admin2 aus `data/raw/hapi/admin2_NPL.json`, Codes landesweit, z. B. Rasuwa `NP0329`, Nuwakot `NP0328`, Dhading `NP0330`), `district_alias` (timure/syabrubesi → NP0329), `source` (Lizenz, default_verification).
...
**Datenfakten, die das Design treiben** (gemessen in `orgs-nepal-2026.json`): 44 Aussagen (gemessen; Web-Lead 28.08.), konzentriert auf Rasuwa/Nuwakot/Dhading, 6 benannte Distrikte nach Normalisierung, ein großer Teil ohne Ortsangabe; 9 von 44 Orgs ohne Aussage; 22 von 44 ohne Finanzzahl; Programmquote 1 von 44. → **Keine Karte, keine Org×Distrikt-Matrix** (3 von 77 Distrikten gefüllt liest sich als „nichts passiert"); die Org-Seite wird um die Abwesenheit herum gestaltet, nicht um eine Tabelle voller Striche.
...
**Gate PO-1:** `ingest_orgs` → 44 Orgs, 14 NP/30 intl, 420 Knoten, **150 mit Wert / 270 Gaps** (Schema v0.3: `mode` nullable, 7 unknown-ohne-Quelle sind echte Gaps), 0 Fehler; zweiter Lauf 0 Zeilen. **PO-2:** Fixture 21/21 verbatim; Live-Lauf über die 35 Flut-Reports, `rejected_unverbatim` < 5 %; ich prüfe 10 Aussagen gegen ihre URL. **PO-3:** `/responders?district=NP0329` (Rasuwa) — jedes Feld source_url oder is_gap; bekannter Gap (NRCS income) explizit; kein body_text; 401 ohne Token; 304 beim zweiten Request. **PO-4:** Security-Review, nur blockierende Findings vor Deploy. **PO-5:** Railway-Deploy beider Services, Healthcheck grün, ein Cron-Lauf in `ingestion_run`, `/meta/freshness` < 1 h.
