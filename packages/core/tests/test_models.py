...
async def test_two_hand_researched_statements_for_the_same_activity_are_rejected(session):
    """The duplication this identity exists to stop.

    response_statement had no identity beyond (report_id, content_hash), and content_hash includes
    the classification. Re-ingesting after the v0.4 reclassification therefore inserted a second
    row for the same researched response and left the stale one beside it - 61 rows where the data
    holds 44 responses. A hand-researched statement is a reload of one record, not a new claim.
    """
    org = await _org(session, org_id="dup-test-org")
    first = await _statement(session, org_id=org.org_id, model="hand_research", activity="distributed tarpaulins")
    with pytest.raises(IntegrityError, match="uq_response_statement_hand_research"):
        await _statement(
            session,
            ...
            model="hand_research",
            activity="distributed tarpaulins",
            activity_type="shelter",
        )


async def test_the_identity_ignores_case_and_whitespace(session):
    """activity_norm is generated by the database, so it cannot disagree with activity."""
    first = await _statement(session, model="hand_research", activity="Distributed  Tarpaulins")
    await session.refresh(first)
    assert first.activity_norm == "distributed tarpaulins"


async def test_two_extracted_statements_from_one_report_are_still_allowed(session):
    """The partial index covers only hand_research. Each extraction is a new claim, and two
    statements about the same organisation in one report are normal."""
    org = await _org(session, org_id="extracted-test-org")
    first = await _statement(session, org_id=org.org_id, model="claude-sonnet-5", activity="deployed a medical team")
    second = await _statement(
        session,
        report_id=first.report_id,
        org_id=org.org_id,
        model="claude-sonnet-5",
        activity="deployed a medical team",
    )
