...
class ResponseStatement(Base):
    ...
    # Lowercased, whitespace-collapsed activity text. A hand-researched statement is a
    # reload of one record, not a new claim, so it needs an identity that survives a change
    # to its classification. Generated by the database so it can never disagree with activity.
    activity_norm: Mapped[str] = mapped_column(
        Computed(r"lower(regexp_replace(btrim(activity), '\s+', ' ', 'g'))", persisted=True)
    )
    ...
    __table_args__ = (
        ...
        # LLM statements: each extraction is a new claim, so content identifies it.
        UniqueConstraint("report_id", "content_hash", name="uq_response_statement_content"),
        # Hand-researched statements: one row per (report, org, activity), whatever the
        # classification says today. Without this, re-ingesting after a reclassification
        # inserted a second row and left the stale one beside it - 61 rows where there are 44
        # responses. Partial, so it constrains only the hand-researched half.
        Index(
            "uq_response_statement_hand_research",
            "report_id",
            "org_id",
            "activity_norm",
            unique=True,
            postgresql_where=text(f"model = '{HAND_RESEARCH_MODEL}'"),
        ),
